...
/**
 * Provides utilities for managing configuration files and creating new ones in others places.
 */
public class ConfigUtils
{
	/**
	 * Creates a new ConfigUtils object. 
	 * @param pluginName Name of the plugin this class is used in. For the logs.
	 * @param logger The logger used to display errors, information, etc.
	 */
	 ...
	 * @return A Config object. If the function is unable to retrieve a config due to an exception, or the config is outdated, returns
	...
	public Config loadConfig(Path path)
	{
		...
		if(rootNode==null)
		{
			logger.error("When trying to load a Config, " + pluginName + " encountered an error. Loading a default configuration file.");
			return defaultConfig;
		}
		
		//Making sure there is a configuration and if not loading a default one.
	...
	/**
	 * Attempts (with no guarantee) to create a default configuration at the path specified. If there is an existing file there, it renames it (pluginname).conf.old
	 * and moves it to the side.
	 * @param path The path of the new configuration.
	 */
	...
	/**
	 * Attempts (with no guarantee) to create a default configuration at the path specified. If there is an existing file there, it renames it (pluginname).conf.(extension)
	 * and moves it to the side.
	 * @param path The path of the new configuration.
	 * @param extension The extension that the old file should be given to make it unusable (if it exists). Name it something appropriate, like .bad if
	 * the file was messed up or .old if it was an old version. Defaults to old if no value is provided.
	 */
	public void newDefaultConfig(Path path, String extensionForOld)
	{
		...
		Preconditions.checkNotNull(extensionForOld, "Tried to create a new default configuration but the passed extensionForOld was null.");
		
		newConfig(path, new Config(), extensionForOld);
	}

	/**
	 * Attempts (with no guarantee) to create a configuration at the path specified. If there is an existing file there, it renames it (pluginname).conf.old
	 * and moves it to the side.
	 * @param path The path of the new configuration.
	 * @param config The configuration to be written to the new file.
	 * the file was messed up or .old if it was an old version. Defaults to old if no value is provided.
	 */
	public void newConfig(Path path, Config config)
	{
		...
		Preconditions.checkNotNull(config, "Tried to create a new configuration but the passed config object was null.");
		
		newConfig(path, config, "old");
	}
	
	/**
	 * Attempts (with no guarantee) to create a configuration at the path specified. If there is an existing file there, it renames it (pluginname).conf.(extension)
	 * and moves it to the side.
	 * @param path The path of the new configuration.
	 * @param config The configuration to be written to the new file.
	 * @param extensionForOld The extension that the old file should be given to make it unusable (if it exists). Name it something appropriate, like .bad if
	 * the file was messed up or .old if it was an old version. Defaults to old if no value is provided.
	 */
	public void newConfig(Path path, Config config, String extensionForOld)
	{
		Preconditions.checkNotNull(path, "Tried to create a new configuration but the passed path object was null.");
		Preconditions.checkNotNull(config, "Tried to create a new configuration but the passed config object was null.");
		Preconditions.checkNotNull(extensionForOld, "Tried to create a new configuration but the passed extensionForOld object was null.");
	...
	/**
	 * Attempts (with no guarantee) to overwrite any existing configuration file at the global path with a default configuration.
	 * @param path The path to write the default configuration to.
	 */
	public void writeDefaultConfig(Path path)
	{
		Preconditions.checkNotNull(path, "Tried to write a default configuration to a path but the passed path object was null.");
		writeConfig(path, new Config());
	}
	
	/**
	 * Attempts (with no guarantee) to overwrite any existing configuration file at the global path with the provided configuration.
	 * @pre path and config are not null
	 * @param path The path to write the configuration to.
	 * @param config The configuration object to write to the path.
	 */
	public void writeConfig(Path path, Config config)
	{
		Preconditions.checkNotNull(path, "Tried to write a configuration to a path but the passed path object was null.");
		Preconditions.checkNotNull(config, "Tried to write a configuration to a path but the passed config object was null.");
		
		ConfigurationNode rootNode = getRootNode(path);
		if(rootNode==null)
		{
			logger.error("We were unable to write the default configuration to file because we were unable to load it.");
		}
		else 
		{
			try
